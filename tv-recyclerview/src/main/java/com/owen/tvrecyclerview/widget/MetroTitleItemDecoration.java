...
public class MetroTitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private static final String TAG = "MetroTitleItemDecoratio";
    ...
    public MetroTitleItemDecoration(Adapter adapter, String[] mData) {
    ...
    private void createAndMeasureTitleView(RecyclerView parent) {
        if(mAdapter == null)
            return;
        Log.e(TAG, "createAndMeasureTitleView: " + mTempItemLp.sectionIndex );
        mTempTitleView = mAdapter.getTitleView(mTempItemLp.sectionIndex, parent);
        int i = 0;
        ((TextView)((LinearLayout)mTempTitleView).getChildAt(0)).setText(data[mTempItemLp.sectionIndex]);
